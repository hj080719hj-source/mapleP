...
function update() {
  ...
  try {
    ...
    $('gold-result').innerHTML=`<h2>평균 필요 골드 큐브</h2><p class="gold-total" id="gold-count">${number(result.attempts)}개</p><dl class="metrics"><div><dt>선택 장비</dt><dd>${selectedItem.name}</dd></div><div><dt>1회 추정 감정비 · 자동</dt><dd id="appraisal-fee">${number(result.appraisalFee)} 메소</dd></div><div><dt>골드 큐브 등업 비용 · 감정비 합계</dt><dd id="appraisal-cost">${money(result.appraisalCost)}</dd></div><div><dt>메소 재설정으로 등업할 때</dt><dd id="mesos-cost">${money(result.mesosCost)}</dd></div></dl><p class="gold-comparison">감정비를 포함하면 골드 큐브 등업으로 평균 ${money(result.savings)}를 절약할 수 있습니다.</p><p class="hint">감정비는 장비 레벨² × 20메소로 추정합니다. 공식 최신 비용표로 확인되지 않은 이용자 공개 산식이며, 보스 처치 횟수·기간은 포함하지 않습니다. <a href="https://gall.dcinside.com/board/view/?id=maplestory&amp;no=9329337" target="_blank" rel="noopener noreferrer">감정비 산식 출처 ↗</a></p>`;
    $('gold-breakdown').innerHTML=`<table><thead><tr><th>등업 구간</th><th>골드 등업 확률</th><th>평균 큐브 수</th><th>추정 감정비 합계</th><th>메소 재설정 비용</th></tr></thead><tbody>${result.rows.map(r=>`<tr><td>${r.name}</td><td>${(r.probability*100).toFixed(4)}%</td><td>${number(r.attempts)}개</td><td>${money(r.appraisalCost)}</td><td>${money(r.mesosCost)}</td></tr>`).join('')}</tbody></table>`;
